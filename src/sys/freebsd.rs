...
const EXTATTR_NAMESPACE_NAMES: [&'static str; 3] = [ "empty", EXTATTR_NAMESPACE_USER_STRING, EXTATTR_NAMESPACE_SYSTEM_STRING ];
const EXTATTR_NAMESPACE_USER: c_int = 1;
const EXTATTR_NAMESPACE_SYSTEM: c_int = 2;

extern "C" {
    pub fn extattr_list_fd(fd: c_int, attrnamespace: c_int, data: *mut c_void, nbytes: size_t) -> ssize_t;
    pub fn extattr_get_fd(fd: c_int, attrnamespace: c_int, attrname: *const c_char, data: *mut c_void, nbytes: size_t) -> ssize_t;
    pub fn extattr_delete_fd(fd: c_int, attrname: c_int, attrname: *const c_char) -> c_int;
    pub fn extattr_set_fd(fd: c_int, attrname: c_int, attrname: *const c_char, data: *const c_void, nbytes: size_t) -> ssize_t;

    pub fn extattr_list_link(path: *const c_char, attrnamespace: c_int, data: *mut c_void, nbytes: size_t) -> ssize_t;
    pub fn extattr_get_link(path: *const c_char, attrnamespace: c_int, attrname: *const c_char, data: *mut c_void, nbytes: size_t) -> ssize_t;
    pub fn extattr_delete_link(path: *const c_char, attrname: c_int, attrname: *const c_char) -> c_int;
    pub fn extattr_set_link(path: *const c_char, attrname: c_int, attrname: *const c_char, data: *const c_void, nbytes: size_t) -> ssize_t;
}
...
impl Iterator for XAttrs {
    ...
    fn next(&mut self) -> Option<OsString> {
        ...
        if self.offset < self.system_attrs.len() {
            Some(prefix_namespace(OsStr::from_bytes(&data[1..siz + 1]), EXTATTR_NAMESPACE_SYSTEM))
        } else {
            Some(prefix_namespace(OsStr::from_bytes(&data[1..siz + 1]), EXTATTR_NAMESPACE_USER))
        }
    }
...
fn name_to_ns(name: &OsStr) -> io::Result<(c_int, CString)> {
    ...
    let nsname = match groups.next() {
        Some(s) => s,
        None => return Err(io::Error::new(io::ErrorKind::InvalidInput, "couldn't find namespace"))
    };

    let propname = match groups.next() {
        Some(s) => s,
        None => return Err(io::Error::new(io::ErrorKind::InvalidInput, "couldn't find attribute"))
    };

    let ns_int = match EXTATTR_NAMESPACE_NAMES.iter().position(|&s| s.as_bytes() == nsname) {
        Some(i) => i,
...
pub fn get_fd(fd: RawFd, name: &OsStr) -> io::Result<Vec<u8>> {
    ...
    unsafe {
        allocate_loop(|buf, len| extattr_get_fd(fd, ns, name.as_ptr(), buf as *mut c_void, len as size_t))
    }
}
...
pub fn list_fd(fd: RawFd) -> io::Result<XAttrs> {
    let sysvec = unsafe {
        let res = allocate_loop(|buf, len| extattr_list_fd(fd, EXTATTR_NAMESPACE_SYSTEM, buf as *mut c_void, len as size_t));
    ...
    let uservec = unsafe {
        let res = allocate_loop(|buf, len| extattr_list_fd(fd, EXTATTR_NAMESPACE_USER, buf as *mut c_void, len as size_t));
        match res {
            Ok(v) => v,
            Err(err) => {
                return Err(err)
...
pub fn get_path(path: &Path, name: &OsStr) -> io::Result<Vec<u8>> {
    ...
    unsafe {
        allocate_loop(|buf, len| extattr_get_link(path.as_ptr(), ns, name.as_ptr(), buf as *mut c_void, len as size_t))
    }
}
...
pub fn list_path(path: &Path) -> io::Result<XAttrs> {
    let path = try!(path_to_c(path));
    let sysvec = unsafe {
        let res = allocate_loop(|buf, len| extattr_list_link(path.as_ptr(), EXTATTR_NAMESPACE_SYSTEM, buf as *mut c_void, len as size_t));
        // On FreeBSD, system attributes require root privileges to view. However,
        // to mimic the behavior of listxattr in linux and osx, we need to query
        // them anyway and return empty results if we get EPERM
        match res {
    ...
    let uservec = unsafe {
        let res = allocate_loop(|buf, len| extattr_list_link(path.as_ptr(), EXTATTR_NAMESPACE_USER, buf as *mut c_void, len as size_t));
        match res {
            Ok(v) => v,
            Err(err) => {
                return Err(err)
